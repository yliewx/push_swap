...
void	init_max_seq(t_data *data, t_seq *max_seq)
{
	max_seq->len = 0;
	max_seq->start = data->stack_a.top;
	max_seq->end = max_seq->start;
	if (data->size > 5)
	{
		max_seq->lis = malloc(data->size * sizeof(int *));
		if (!max_seq->lis)
			end(data, "Error allocating memory for LIS array.\n", 1);
	}
}
...
int	main(int argc, char **argv)
{
	...
	init_max_seq(&data, &data.max_seq);
	if (data.stack_a.size == 2)
		swap(&data.stack_a, false);
	else if (data.stack_a.size == 3)
		sort_3(&data.stack_a);
	else if (data.stack_a.size > 3 && data.stack_a.size <= 5)
		sort_5(&data.stack_a, &data.stack_b, &data.max_seq);
	else
		sort_advanced(&data, &data.stack_a, &data.stack_b);
	end(&data, NULL, 0);
	return (0);
}
